      ...
      (entries) => {
        if (entries[0].isIntersecting) {
          ...
          items.forEach((item, index) => {
            setTimeout(() => {
              (item as HTMLElement).classList.add('opacity-100', 'translate-y-0', 'scale-100');
              (item as HTMLElement).classList.remove('opacity-0', 'translate-y-10', 'scale-90');
            }, 100 * index);
          });
        }
      },
      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        >
          {certificatesData.map((certificate) => (
            <div 
              key={certificate.id}
              className="certificate-card opacity-0 translate-y-10 scale-90 transition-all duration-700"
              onClick={() => openModal(certificate)}
            >
              <div className="relative h-48 group overflow-hidden rounded-lg bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/30 dark:to-blue-800/20 p-6 hover:shadow-[0_0_20px_rgba(58,134,255,0.4)]">
                {/* Award Icon with Enhanced Animation */}
                <div className="absolute top-3 right-3 z-10 certificate-icon-wrapper">
                  <Award className="certificate-award-icon text-primary w-6 h-6" />
                </div>
              ...
              <div className="flex justify-between items-start mb-6">
                <div className="flex items-center gap-3">
                  <div className="certificate-icon-wrapper">
                    <Award className="certificate-award-icon text-primary w-8 h-8" />
                  </div>
